...
每个响应都带有 **`X-Trace-ID`** 头——一个贯穿整条调用链的 W3C 兼容 trace id。请把它与您自己的请求 id 一起记录下来；一旦出现异常，这是技术支持定位具体调用的最快方式。
...
| `502 Bad Gateway` | 记忆网关无法访问其上游服务。 | 带退避重试；若持续出现，请携带您的 `X-Trace-ID` 联系 [contact@data.cloud](mailto:contact@data.cloud)。 |
