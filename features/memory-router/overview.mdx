...
<Note>
Memory Router 在 MemoryLake **中国站暂未开放**。中国站部署尚未启用该记忆网关：`/openai/v1` 与 `/anthropic/v1` 端点当前无法调用，控制台 **Integrations** 下也不会出现 Memory Router 卡片。本章节保留完整说明，供您提前评估接入方案；如需在中国站使用，请联系商务了解开放计划。中国站当前可用的记忆接入方式是 [REST API](/features/memorylake/api-reference/overview)、[MCP 服务器](/features/memorylake/mcp-servers/creating-api-keys)与 [Agent 插件](/features/memorylake/mcp-servers/openclaw-plugin)。
</Note>
...
| **按 Boundary 划定范围** | [Boundary](#boundaries-the-memory-scope) 精确定义一段会话的记忆归属于哪个工作空间、哪些项目和哪些 Actor。 |
