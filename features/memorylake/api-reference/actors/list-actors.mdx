...
按标签筛选。重复传参（`?tags=vip&tags=cn`）或用逗号分隔（`?tags=vip,cn`）可同时要求多个标签——只返回同时带有所有列出标签的 Actor。匹配是精确且区分大小写的。
</ParamField>
...
```bash 按标签筛选
curl -X GET 'https://app.memorylake.cn/openapi/memorylake/api/v3/actors?tags=vip&tags=cn' \
  -H 'Authorization: Bearer sk_xxxxxx'
```
