...
用于分组和筛选 Actor 的标签，例如 `vip` 或 `cn`。重复的标签会被去除，首尾空白会被裁剪。匹配是精确且区分大小写的，因此 `VIP` 和 `vip` 是两个不同的标签。每个 Actor 最多 20 个标签，每个标签最长 64 个字符，标签中不能包含逗号。
</ParamField>

<ParamField body="metadata" type="object">
任意键值对，用于调用方定义的元数据，例如层级、地区或部门。元数据不支持筛选——需要按属性检索时请使用 `tags`。
</ParamField>
