...
<ParamField body="actor_ids" type="array" required>
参与会话的 Actor ID 列表，至少需要一个。每条消息都归属于某个 Actor，Actor 的记忆也由其发言构建，因此没有 Actor 的会话既分不清参与方，也无法为任何人建立记忆。每个 ID 必须引用您已创建、并已绑定到该工作空间的 Actor。
</ParamField>
