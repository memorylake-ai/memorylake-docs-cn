...
# 首条消息的 parent_message_id 填 null，之后每条填上一次追加返回的 id
curl -X POST "$BASE/api/v3/conversations/$CONVERSATION_ID/messages" \
  -H "Authorization: Bearer $MEMORYLAKE_API_KEY" \
  -H "Content-Type: application/json" \
  -d "{\"custom_id\": \"msg-001\", \"actor_id\": \"$ACTOR_ID\", \"parent_message_id\": null, \"content\": [{\"block_type\": \"TEXT\", \"text\": \"I am vegetarian and lactose intolerant\"}]}"
